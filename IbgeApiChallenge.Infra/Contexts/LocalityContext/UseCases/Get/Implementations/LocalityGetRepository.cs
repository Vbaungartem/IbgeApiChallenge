...
public class LocalityGetRepository : ILocalityGetRepository
{
    ...
    public async Task<Locality?> GetByIbgeCodeAsync(string ibgeCode, CancellationToken cancellationToken)
    {
        return await GetByPredicate(locality => locality.IbgeCode == ibgeCode, cancellationToken);
    }

    public async Task<Locality?> GetByIdAsync(string id, CancellationToken cancellationToken)
    {
        return await GetByPredicate(locality => locality.Id == new Guid(id), cancellationToken);
    }

    public async Task<Locality?> GetByNameCodeAsync(string name, CancellationToken cancellationToken)
    {
        return await GetByPredicate(locality => locality.Name == name, cancellationToken);
    }

    private async Task<Locality?> GetByPredicate(Expression<Func<Locality, bool>> predicado, CancellationToken cancellationToken)
    {
        try
        {
            return await _context.Locality.FirstOrDefaultAsync(predicado, cancellationToken);
        }
        catch (Exception e)
        {
